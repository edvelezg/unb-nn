...
for different neurons, allowing emphasis on certain features of the patterns processed. Neurons are also often grouped together in
layers or slabs. Neural networks may be composed of 1 or more layers. These neurons work in parallel to perform various complex tasks.
Each Neuron aggregates input that is scales the synaptic weights and calculates an output with an activation function. The inputs to
the neuron may be outputs from other neurons or the environment.

Neural networks possess important advantages and capabilities as computational models those include their capability to solve problems
of nonlinear nature, input-output mapping and adaptivity~\cite{Haykin:1994:NNC:541500}. They have also been applied successfully in the
...
The processing of a neural network begins when an external pattern made up of signal is captured by an input layer.
...
The feed forward neural network begins with an input layer which may be connected to a hidden layer or directly to an output layer. In
a feedforward neural network adjacent layers of neurons are interconnected. The first layer is the input layer which directly connects
to the external environment through which the input patterns are presented to the network. The last layer is the output layer which
produces the output pattern to the external environment. All other layers are considered hidden layers, and may or may not be present.
The input layer provides the means through which the external environment can affect it.
...
The Backpropagation algorithm has time complexity within $O(n)$, which is faster than the RMS Minimization algorithm with a time complexity of $O(n^{2})$ when varying the number of neurons in the middle layer.
The Backpropagation algorithm reached a smaller error for the simple examples that the RMS Minimization algorithm.
As observed in some results, the neural network may get stuck in local minima unable to arrive to better solutions. We noticed this in both the Backpropagation and RMS Minimization algorithms.
