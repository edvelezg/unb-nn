...
The overall behavior of the neural network is determined by the individual behavior of each neuron. A neuron gathers input from the
external environment or neighboring neurons that are connected to it. It then aggregates the modulated input and passes that information
...
considered hidden and may or may not be present. The signal received is modulated by the weighting value of the connection. The
modulated inputs are then integrated. (summed). An activation value is determined. (I don’t understand how? Separate function F they
don’t talk much about.) ai(t) = Fi(ai(t-1), net_i(t)). F is not activation function!
...
Algorithm~\ref{alg:feedforward}). Each unit receives stimulation from other units, except the input layer.
