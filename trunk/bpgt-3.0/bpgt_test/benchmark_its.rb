...
def run_with_cnt_nrns(iterations, seed, norm_file, nrn_cnt)
  ...
  net = NeuralNetwork::Backpropagation.new([2, nrn_cnt, 1])
...
Benchmark.bm do |x|
  for num in 1..20
    ...
    x.report("iterations:#{iterations*num} ") { info << run_with_cnt_nrns(iterations*num, 5, norm_file, nrn_cnt) }
  end
end
