...
def run_with_iterations(iterations, seed, norm_file, cnt_nrns)
...
Benchmark.bm do |x|
  for num in 1..20
    ...
    x.report("iterations:#{num*iterations}") { info << run_with_iterations(num*iterations, 10, norm_file, cnt_nrn) }
  end
...
rs_f = File.open("#{File.basename(__FILE__, ".rb")}.txt", "w")
