...
We examine two different algorithms to train the feedforward neural network. One algorithm is the Back-propagation algorithm and the other is the Root Mean
Square (RMS) Minimization algorithm.
...
inputs and target outputs to the network. We run the feedforward algorithm with each set example and generate the corresponding outputs from the neural network. After generating the outputs we calculate the root mean square between the output given by the neural network and the target output provided in the example. Then a small change is made to one of the weights of the neural network, and the feedforward algorithm is used again to calculate outputs with the change. The RMS algorithm then change the output. the presented inputs in our neural network.
...
Our procedures involve a set of system experiments where we test the RMS and back-propagation training methods. We compare the time complexity between both
training methods as well as their accuracy after being trained. The set of examples to train the network were generated by taking two numerical inputs and adding
...
\begin{figure}[htbp]
	\centering
		\includegraphics[height=3in]{../docs/TimeNrns.pdf}
	\caption{Comparison between back-propagation and RMS minimization learning algorithms in time as we increase the number of neurons in the middle layer. Note that back-propagation has linear growth, and RMS minimization has quadratic growth.}
	\label{fig:docs_TimeNrns}
\end{figure}
