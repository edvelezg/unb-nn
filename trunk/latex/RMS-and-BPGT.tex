...
output obtained through equation \ref{eqn:damping}. To further test the network, we give it initially positions $x(1)$ and $x(0)$. The network then predicts an $x(2)$ position. It then uses the output and previous information to predict $x(3)$. This process continues recursively all positions for all time steps are generated (see Figure~\ref{fig:bpgt-3.0_damping_test_final_output}). Comparing Figures~\ref{fig:bpgt-3.0_damping_test_output} and \ref{fig:bpgt-3.0_damping_test_final_output} we notice that the predicted output from the neural network deviates more from the expected output as time is advanced. \textbf{How do I explain that there is some error accumulation}.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.49\columnwidth]{../bpgt-3.0/damping_test/Trainings.pdf}
...
% So far we have the neural network learning with the backpropagation algorithm. We should include an image of the training curve for it. We should talk about the seed that performed the best and how we save the configuration. 
% 
% Also how we tested the neural network with some perfect data to predict the next time step and. Oh should mention the number of iterations we used to train the network. So a nice picture comparing the perfect data with another picture comparing the self generated data. Finally we talk about how the neural network has no problems in generating new data when starting at different points of the curve
