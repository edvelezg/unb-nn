...
root mean square between outputs given by the neural network and target output provided by the example. We keep this calculation as $\mbox{RMS}_{\mbox{old}}$.
...
We then calculate a slope by taking the difference between $\mbox{RMS}_{\mbox{old}}$ and $\mbox{RMS}_{\mbox{new}}$, and divide by the change
we made to the weight.
...
Looking at the time complexity analytically we c5ount the number of operations made by both algorithms. To determine the number of
