...
Here we talk about the difference in training accuracy between the back-propagation and RMS minimization algorithms. To test the
...
combinations, thus the network is trained on a sample of $121$ different sets of inputs and outputs.
...
Table~\ref{tab:training_sessions}. Overall the back-propagation algorithm trained the neural network to a higher degree of accuracy. A comparison between the expected output and the output produced by the neural networks after training with back-propagation and RMS minimization are illustrated in Figures\ref{fig:rms-w-alpha_graphs_output-9} and \ref{fig:bpgt-3.0_bpgt_test_output-5} respectively.
...
of a spring that is connected to a fixed object. Ideally without the effect of friction and damping, the system becomes
an ideal harmonic oscillator. However if we add the effect of damping the system will eventually run out of energy and converge to zero.  To simulate this physical phenomenon we used the
following equation:
